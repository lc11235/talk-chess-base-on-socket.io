    ...
    */
    ...
    factory.ajax = function (options, _this) {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState == 4) {
                if (factory.settings.DEBUG) {
                    console.info(_MODULE_NAME, "request status:", xhr.status);
                }
        ...
        if (key && value) {
            ...
            if (options !== null) {
                if (options.expires) {
                    ...
                    cookie += ';expires=' + times.toUTCString();
                }
                if (options.path) {
    ...
    /**
     * [getAllCookie 获取当前全部cookie]
     * @return {[object]} [包含cookie的一个对象]
     */
    factory.getAllCookie = function () {
        var cookies = parseCookie();
        var tmpCookies = {};
        for (var key in cookies) {
    ...
    var parseCookie = function () {
        var cookies = {};
        if (factory.settings.DEBUG) {
            console.log("[INFO]" + _MODULE_NAME + ": Cookie information:");
            console.log(document.cookie);
        }
        if (document.cookie) {
